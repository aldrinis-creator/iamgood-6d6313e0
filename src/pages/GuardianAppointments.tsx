...
import { CalendarDays, Clock, MapPin, Plus, Pencil, Trash2, Bell, Hourglass, UserCircle } from "lucide-react";
import {
...
const GuardianAppointments = () => {
  ...
  const { data: appointments = [], isLoading } = useQuery({
    queryKey: ["appointments", "guardian", wardUserId],
    queryFn: async () => {
  ...
  const filtered = useMemo(() => {
    return filter === "today"
      ? appointments.filter((a: any) => isToday(parseISO(a.start_date)))
      : appointments;
  }, [appointments, filter]);

  const todayCount = appointments.filter((a: any) => isToday(parseISO(a.start_date))).length;
      ...
      <div className="p-4 pb-24 space-y-4">
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            Today {todayCount > 0 && (
              <Badge variant="secondary" className="ml-1 text-xs bg-destructive-foreground text-destructive">
                {todayCount}
              </Badge>
            )}
          </Button>
        ...
        {!wardUserId ? (
          <Card className="p-8 text-center text-muted-foreground">
            Select a ward to view their appointments.
          </Card>
        ) : isLoading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {filtered.map((apt: any) => {
              ...
              const addedByGuardian = apt.created_by === session?.user?.id;
              const authorLabel = addedByGuardian
                ? "Added by you"
                : `Added by ${wardName}`;
              return (
                <Card
                  key={apt.id}
                  className={`p-4 space-y-3 overflow-hidden min-w-0 ${isDueToday ? "border-destructive border-2 shadow-[0_0_8px_hsl(var(--destructive)/0.3)]" : ""}`}
                >
                  ...
                  <div className="text-sm text-muted-foreground space-y-1">
                    <p className="flex items-center gap-1 flex-wrap">
                      ...
                      {format(parseISO(apt.start_date), "EEE, MMM d")} at {apt.start_time?.slice(0, 5)}
                    </p>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove the appointment from {wardName}'s schedule. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
