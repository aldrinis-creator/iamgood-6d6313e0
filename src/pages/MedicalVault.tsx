...
const MedicalVaultContent = () => {
  ...
  const buildRecordViewHtml = (r: MedicalRecord) => {
    const shareText = [
      `📋 ${r.record_type}: ${r.title}`,
      r.record_date ? `Date: ${new Date(r.record_date).toLocaleDateString("en-IN")}` : "",
      r.doctor_name ? `Doctor: ${r.doctor_name}` : "",
      r.hospital_name ? `Hospital: ${r.hospital_name}` : "",
      r.description ? `\nNotes:\n${r.description}` : "",
      "",
      "Generated by Check-iN Health Vault",
    ].filter(Boolean).join("\n");
    const whatsAppUrl = `https://wa.me/?text=${encodeURIComponent(shareText)}`;
    const emailSubject = encodeURIComponent(`${r.record_type} — ${r.title}`);
...
<div class="action-bar" id="actionBar">
  ...
  <button onclick="window.location.href='${emailUrl}'" class="btn btn-email">📧 Email</button>
    ...
    const bodyHtml = `
<div class="section"><div class="section-title">📋 Record Details</div><div class="section-body">
<div class="row"><span class="label">Title</span><span class="value">${r.title}</span></div>
<div class="row"><span class="label">Type</span><span class="value">${r.record_type}</span></div>
${r.record_date ? `<div class="row"><span class="label">Date</span><span class="value">${new Date(r.record_date).toLocaleDateString("en-IN")}</span></div>` : ""}
${r.doctor_name ? `<div class="row"><span class="label">Doctor</span><span class="value">${r.doctor_name}</span></div>` : ""}
...
${r.file_name ? `<div class="section"><div class="section-title">📎 Attachment</div><div class="section-body"><p>${r.file_name}</p></div></div>` : ""}`;
  ...
  const openRecordViewWindow = (r: MedicalRecord) => {
