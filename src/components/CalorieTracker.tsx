...
import { format, subDays, startOfDay, addDays, startOfWeek, endOfWeek, eachDayOfInterval } from "date-fns";
...
const CalorieTracker = () => {
  ...
  // Fetch calorie goal from nutrition persona
  useEffect(() => {
    if (!user) return;
    supabase
  ...
  // Fetch logs for chart (past 30 days)
  useEffect(() => {
    if (!user) return;
    setLoading(true);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {dayLogs.length === 0 ? (
          <p className="text-xs text-center text-muted-foreground py-4">No meals logged {isToday ? "today" : "this day"}. Use "Analyze This Meal" to scan and save.</p>
        ) : (
          dayLogs.map((log) => (
            <Card key={log.id}>
