...
4. It can reappear up to 3 times in total for the same missed Check-iN (on the next scan cycle after each auto-close). the 3rd pop-up stays on until Guardian dismisses it. 
...
show #3 -> blink 5s + 1 audio -> visible 60s -> opne until Guardian dismisses it
```
