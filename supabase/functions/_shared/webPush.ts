// Shared VAPID web-push sender (same approach as send-checkin-push).
...
export async function pushToUser(sb: any, userId: string, payload: object): Promise<number> {
  ...
  for (const sub of subs || []) {
    try {
      const res = await sendPushNotification(sub, payload, VAPID_PUBLIC_KEY, priv, VAPID_SUBJECT);
      if (res.ok) sent++;
      else if (res.status === 410 || res.status === 404) await sb.from("push_subscriptions").delete().eq("endpoint", sub.endpoint);
