...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!res.ok) {
      console.error("MSG91 WhatsApp safe_zone failed", res.status, text);
    } else {
      console.log("MSG91 WhatsApp safe_zone sent to", phones.length, "recipients");
    }
    ...
    console.error("msg91-whatsapp-safezone error:", err);
